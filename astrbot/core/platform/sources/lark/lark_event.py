...
from astrbot.api.message_components import At, AtAll, File, Json, Plain, Record, Video
...
class LarkMessageEvent(AstrMessageEvent):
    ...
    async def _convert_to_lark(message: MessageChain, lark_client: lark.Client) -> list:
        ret = []
        _stage = []
        # Feishu post: `md` cannot share a row with `at`; use `text` when @ is present.
        has_at_components = any(isinstance(c, (At, AtAll)) for c in message.chain)
        plain_tag = "text" if has_at_components else "md"
        for comp in message.chain:
            if isinstance(comp, Plain):
                _stage.append({"tag": plain_tag, "text": comp.text})
