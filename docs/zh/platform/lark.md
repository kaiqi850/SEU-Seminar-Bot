...
| 文本 | 是 | 是 | |
| 图片 | 是 | 是 | |
| 语音 | 是 | 是 | 接收需 `im:resource` 权限 |
| 视频 | 是 | 是 | 接收需 `im:resource` 权限 |
| 文件 | 是 | 是 | 接收需 `im:resource` 权限 |
...
- `/at 顾凯琪` 或 `/艾特 顾凯琪`：@ 指定成员（按群内显示名称匹配）
- `/at 顾凯琪 请到会议室`：@ 成员并附带消息
- `/atall` 或 `/全员`：@ 全体成员

### 对话方式

@ 机器人后可直接说「帮我艾特顾凯琪」，机器人会调用内置工具完成 @（需模型支持 function calling / tool use，且已开启对应能力）。

### 权限

按姓名 @ 成员需要开通 **查看群成员**（`im:chat.members:read`）或 **获取群组信息**（`im:chat:readonly`）权限。

将 `<APP_ID>` 替换为你的应用 App ID 后打开，可一键申请：

```text
https://open.feishu.cn/app/<APP_ID>/auth?q=im:chat.members:read,im:chat:readonly&op_from=openapi&token_type=tenant
```

开通后须在开发者后台 **创建并发布新版本**，权限才会对线上机器人生效。

## 群聊文件上传通知

若希望机器人在群成员上传文件时自动提醒群成员，可在 AstrBot 管理面板中编辑飞书机器人配置，开启 **「群聊收到文件时自动通知成员」**（`file_notify_on_upload`）。

可选开启 **「文件通知时 @ 全体成员」**（`file_notify_at_all`），通知消息将 @ 群内所有人。

> [!IMPORTANT]
> 此功能需要机器人能收到群聊中的文件消息，请确保已开通 `im:message.group_msg` 与 `im:resource` 权限，并将应用发布到最新版本。

也可在 `data/cmd_config.json` 的对应飞书平台配置中手动添加：

```json
"file_notify_on_upload": true,
"file_notify_at_all": true
```
