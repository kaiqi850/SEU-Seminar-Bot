...
class LarkPlatformAdapter(Platform):
    ...
    def _build_message_str_from_components(
        ...
        for idx, comp in enumerate(components):
            if isinstance(comp, Comp.Plain):
                ...
                if idx == 0 and any(
                    isinstance(next_comp, Comp.Plain) and next_comp.text.strip()
                    for next_comp in components[idx + 1 :]
                ):
                    continue
        ...
        if message_type in ("post", "image"):
            ...
            for comp in comp_list:
                ...
                if tag == "at":
                    ...
                    elif user_key in at_map:
                        components.append(at_map[user_key])
                    elif user_key:
                        components.append(
                            Comp.At(
                                qq=user_key,
                                name=str(comp.get("user_name", "") or user_key),
                            ),
                        )
                elif tag == "text":
    ...
    async def convert_msg(self, event: lark.im.v1.P2ImMessageReceiveV1) -> None:
        ...
        parsed_components = self._inject_mentions_for_attachment_messages(
            message_type=message.message_type or "unknown",
            ...
            for mention in message.mentions:
                if (
                    ...
                    if sender_nickname:
                        break
        if not sender_nickname:
            sender_nickname = event.event.sender.sender_id.open_id[:8]
        abm.sender = MessageMember(
            user_id=event.event.sender.sender_id.open_id,
            nickname=sender_nickname,
        )
        if abm.type == MessageType.GROUP_MESSAGE and abm.group_id:
            remember_chat_member_name(
                abm.group_id,
        ...
        await self._maybe_notify_file_upload(abm)
        await self.handle_msg(abm)

    @staticmethod
    def _inject_mentions_for_attachment_messages(
        *,
        message_type: str,
        components: list[Comp.BaseMessageComponent],
        at_map: dict[str, Comp.At],
    ) -> list[Comp.BaseMessageComponent]:
        """Inject @ mentions into file/audio/media messages for waking and handlers."""
        if message_type not in ("file", "audio", "media") or not at_map:
            return components

        existing_at_ids = {
            str(comp.qq) for comp in components if isinstance(comp, Comp.At)
        }
        mention_components: list[Comp.BaseMessageComponent] = []
        for at_comp in at_map.values():
            if str(at_comp.qq) not in existing_at_ids:
                mention_components.append(at_comp)
        if not mention_components:
            return components
        return [*mention_components, *components]

    async def _maybe_notify_file_upload(self, abm: AstrBotMessage) -> None:
        """Notify group members when a file is uploaded, if enabled in platform config."""
        if not self.config.get("file_notify_on_upload", False):
            return
        if abm.type != MessageType.GROUP_MESSAGE or not abm.group_id:
            return

        files = [comp for comp in abm.message if isinstance(comp, Comp.File)]
        if not files:
            return

        sender_label = (
            abm.sender.nickname if abm.sender and abm.sender.nickname else "群成员"
        )
        file_names = ", ".join(str(f.name or "未命名文件") for f in files)
        notify_text = f"📎 {sender_label} 上传了文件：{file_names}"

        chain = MessageChain().message(notify_text)
        if self.config.get("file_notify_at_all", True):
            chain.at_all()
        ...
        try:
            await LarkMessageEvent.send_message_chain(
                chain,
                self.lark_api,
                receive_id=abm.group_id,
                receive_id_type="chat_id",
            )
        except Exception as e:
            logger.error(f"[Lark] 文件上传群通知失败: {e}", exc_info=True)
