...
| Text | Yes | Yes | |
| Image | Yes | Yes | |
| Voice | Yes | Yes | Receiving requires `im:resource` permission |
| Video | Yes | Yes | Receiving requires `im:resource` permission |
| File | Yes | Yes | Receiving requires `im:resource` permission |
...
## Group File Upload Notifications

To notify group members when someone uploads a file, enable **Notify members on file upload in group** (`file_notify_on_upload`) in the Lark bot settings in the AstrBot dashboard.

You can optionally enable **@ all members in file notifications** (`file_notify_at_all`).

> [!IMPORTANT]
> This requires the bot to receive file messages in group chats. Make sure `im:message.group_msg` and `im:resource` permissions are granted and the app version is published.
